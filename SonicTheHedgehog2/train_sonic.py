# train_sonic.py
# PPO training for Sonic 2 with extra state features and shaped rewards.
...
def to_chw(obs_np):
    """Convert env obs (HWC or HW) to CHW float32 in [0,1]."""
    if obs_np.ndim == 2:
        chw = obs_np[None, :, :]
...
def compute_gae(rewards, values, dones, next_value, gamma=0.99, lam=0.95):
    """Standard GAE (vectorized over time)."""
    T = len(rewards)
...
def entropy_coef_schedule(total_steps_done: int) -> float:
    """Linearly decay 0.05 -> 0.01 over first 200k steps, then hold."""
    prog = min(total_steps_done / 200_000.0, 1.0)
    return 0.05 - 0.04 * prog
...
def latest_checkpoint(path="checkpoints"):
    ...
    for f in files:
        ...
        try:
            n = name.split("_")[-1].replace(".pt", "")
            steps = int(n[:-1]) * 1000 if n.endswith("k") else int(n)
        except Exception:
            continue
        if steps > best_steps:
            best_steps, best = steps, f
    # fallback to explicit "latest"
    if best is None:
        latest = glob.glob(os.path.join(path, "sonic_ppo_latest.pt"))
...
def main():
    # ---- Env ----
    env = make_env(render='rgb_array')  # don't pass unsupported kwargs
    ...
    print(f"✅ Training | obs_shape={(obs_channels, IMG_SIZE, IMG_SIZE)} | actions={num_actions}")
    ...
    while global_steps < TOTAL_TIMESTEPS:
        ...
        for t in range(rollout_steps):
            ...
            if (global_steps % 200) == 0:
                print(f"… collecting | steps={global_steps:,} | x={info.get('x',0)} | R_ep={ep_return:.1f}")

            if done:
                episode += 1
                print(f"[ep {episode:04d}] return={ep_return:.2f} | steps={global_steps:,}")

                # schedule next episode video on save ticks
                if (global_steps // SAVE_FREQ) != ((global_steps - 1) // SAVE_FREQ):
                    record_next_episode = True

                out = env.reset()
                obs1, info = out if isinstance(out, tuple) else (out, {})
                obs_chw = to_chw(obs1)
                ep_return = 0.0
        ...
        for _ in range(epochs):
            for mb_obs, mb_extra, mb_act, mb_oldlogp, mb_ret, mb_val, mb_adv in minibatches(
                ...
                # final loss with entropy schedule
